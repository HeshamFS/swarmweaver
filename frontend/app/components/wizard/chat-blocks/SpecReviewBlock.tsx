...
interface SpecReviewBlockProps {
  isLoading: boolean;
...
export default function SpecReviewBlock({
  isLoading,
  generatedSpec,
  projectDir,
  onApprove,
  ...
  // Auto-scroll spec content while streaming
  useEffect(() => {
    if (isLoading && generatedSpec && contentRef.current) {
      contentRef.current.scrollTop = contentRef.current.scrollHeight;
    }
  }, [generatedSpec, isLoading]);

  const lineCount = useMemo(() => editedSpec.split("\n").length, [editedSpec]);
  const isModified = editedSpec !== generatedSpec;

  const handleApprove = async () => {
    if (isModified && generatedSpec) {
      await onSaveSpec(projectDir, editedSpec);
    }
    onApprove();
  };
    ...
    return (
      <div className="border border-[#222] bg-[#121212] mb-3 font-mono">
        <div className="flex items-center gap-3 px-4 py-2">
          ...
          <span className="text-[#555] text-xs font-bold uppercase tracking-wider">Spec Approved</span>
  ...
  const isStreamingPartial = isLoading && !!generatedSpec;

  if (isLoading && !generatedSpec) {
    return null;
  }
  ...
  return (
    <div className="border border-[#333] bg-[#121212] mb-3 font-mono">
      ...
      <div className="flex items-center gap-3 px-4 py-2 border-b border-[#222] bg-[#0C0C0C]">
        ...
        <span className="text-[#E0E0E0] text-xs font-bold uppercase tracking-wider">Project Specification</span>
        {isStreamingPartial && (
      ...
      <div ref={contentRef} className="relative max-h-[400px] overflow-y-auto tui-scrollbar">
        {isEditing ? (
          ...
          <div className="p-4 text-xs">
            <MarkdownPreview>{editedSpec}</MarkdownPreview>
